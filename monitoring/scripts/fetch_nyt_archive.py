...
from windows import EVENT_WINDOWS, CALM_WINDOWS  # noqa: E402

RAW_DIR = MONITORING_ROOT / "news" / "data" / "nyt_raw"
API_URL = "https://api.nytimes.com/svc/archive/v1/{year}/{month}.json?api-key={key}"
SLEEP_S = 12.5  # 5 req/min limit

# Calm-window 3-month samples used for Week-3 triage threshold sanity checks.
CALM_SAMPLE_MONTHS = [(2005, 3), (2005, 4), (2005, 5), (2013, 3), (2013, 4), (2013, 5)]
...
def required_months(full_calm: bool) -> list[tuple[int, int]]:
    ...
    if full_calm:
        ...
        months.update(CALM_SAMPLE_MONTHS)
    return sorted(months)


def fetch_month(year: int, month: int, key: str) -> Path:
    ...
    with urllib.request.urlopen(req, timeout=120) as resp:
        payload = resp.read()
    # Validate before caching so a rate-limit error page never poisons the cache.
    docs = json.loads(payload)["response"]["docs"]
...
def main():
    ...
    months = required_months(args.full_calm)
    todo = [(y, m) for y, m in months if not (RAW_DIR / f"{y:04d}-{m:02d}.json.gz").exists()]
    ...
    for y, m in months:
        try:
            ...
            print(f"  {y:04d}-{m:02d}: FAILED ({e}) — rerun the script to resume")

    have = sorted(p.name for p in RAW_DIR.glob("*.json.gz"))
    missing = [f"{y:04d}-{m:02d}" for y, m in months
               if f"{y:04d}-{m:02d}.json.gz" not in have]
    print(f"\nCached months: {len(have)}; missing: {missing or 'none'}")
    if not missing:
        print("Next: python scripts/build_news_cache.py")
