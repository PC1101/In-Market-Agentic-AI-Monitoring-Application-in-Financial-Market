"""
run_sector_momentum_longonly.py — JT Momentum 12-1 LONG-ONLY per GICS sector (ASX 200).

Top quintile only (no short leg). Weights sum to 1.0 (fully invested long).
Each sector starts at $1M independently.

Output: AusEquities/results/sector_momentum_longonly/
"""
...
from run_sector_statarb import load_sector_benchmark, INITIAL_VALUE


def build_sector_map(symbols):
    """GICS sector map — strips .au suffix for Norgate classification API."""
    by_sector = defaultdict(list)
    for sym in symbols:
        sym_clean = sym.split('.')[0] if '.' in sym else sym
        try:
            sector = nd.classification_at_level(sym_clean, "GICS", "Name", 1)
            if sector:
...
def main():
    log.info("=== JT Momentum LONG-ONLY — Sector Analysis (ASX 200) ===")
    ...
    if results:
        ...
        ax.plot(bm_aligned.index, bm_aligned.values, color="black", linewidth=2.0,
                linestyle="--", label=f"ASX 200 EW B&H  ({bm_cagr:+.0%} CAGR)", zorder=10)
    ...
    ax.set_title("JT Momentum 12-1 LONG-ONLY — Net by GICS Sector (ASX 200 C&P, $1M each, after TC)",
    ...
    fig2.suptitle("JT Momentum 12-1 LONG-ONLY — Per Sector (ASX 200 C&P, $1M Start, net of TC)",
                  fontsize=13, fontweight="bold")
    fig2.savefig(OUT_DIR / "summary_grid.png", dpi=180, bbox_inches="tight")
    plt.close(fig2)

    # ── Summary table ─────────────────────────────────────────────────────────
    print("\n=== JT Momentum LONG-ONLY — ASX 200 Sector Results ===")
