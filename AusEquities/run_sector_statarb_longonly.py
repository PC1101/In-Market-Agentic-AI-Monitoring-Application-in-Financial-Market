"""
run_sector_statarb_longonly.py — AL PCA Stat Arb LONG-ONLY per GICS sector (ASX 200).

Long positions only (s-score < -1.25, close > -0.50). No short leg, no borrow cost.
Each sector starts at $1M independently.

Output: AusEquities/results/sector_statarb_longonly/
"""
...
from run_sector_statarb import (
    compute_sector_pca, load_sector_benchmark,
...
def main():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    log.info("=== AL PCA Stat Arb LONG-ONLY — Sector Analysis (ASX 200) ===")
    ...
    fig.suptitle("AL PCA Stat Arb LONG-ONLY — Per Sector (ASX 200 C&P, $1M Start, net of TC)",
