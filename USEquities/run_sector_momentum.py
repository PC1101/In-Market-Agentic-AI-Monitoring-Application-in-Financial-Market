"""
run_sector_momentum.py — JT Momentum 12-1 run independently within each GICS sector (S&P 500).

Each sector starts at $1M independently, compared against SPDR sector ETF benchmark.

Output: USEquities/results/sector_momentum/
"""
...
def run_sector_jt(sector, tickers, prices, membership):
    ...
    # ffill weights so skipped months inherit previous holdings (no flatlines)
    ...
    # Rebase both by GROSS starting value so initial entry TC is preserved
...
def main():
    log.info("=== JT Momentum — S&P 500 Sector Analysis ===")
    ...
    for i, (sector, gross_eq, net_eq, cagr, total_tc) in enumerate(results):
        ...
        ax.plot(net_eq.index, net_eq.values, color=colour, linewidth=1.3,
    ...
    if results:
        ...
        ax.plot(bm_aligned.index, bm_aligned.values,
                ...
                label=f"S&P 500 EW B&H  ({bm_cagr:+.0%} CAGR)", zorder=10)
    ...
    ax.set_title(
        ...
        "(S&P 500 C&P, each sector independent $1M, after all TC)",
    ...
    fig2.suptitle(
        "JT Momentum 12-1 — Per Sector  (S&P 500 C&P, $1M Start, net of TC)",
        fontsize=13, fontweight="bold",
    )
    ...
    # ── Summary table ─────────────────────────────────────────────────────────
    print("\n=== JT Momentum — S&P 500 Sector Results ===")
