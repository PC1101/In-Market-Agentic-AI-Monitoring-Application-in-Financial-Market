...
def extract_trades(pos_df, ret_df, direction):
    '''
    Reconstruct individual trades (contiguous holding runs) from a 0/1 position df.
    :param pos_df: position df (index Date, columns tickers, 1 while holding)
    :param ret_df: actual single-stock returns df (ticker columns) to value trades
    :param direction: 'long' or 'short'; a short trade profits when the price falls
    :return: trade blotter DataFrame [ticker, side, entry, exit, days, trade_ret]
    '''
    ...
    for ticker in pos_df.columns:
        ...
        ## actual returns aligned to the position dates; missing -> 0 (no move)
        ...
        for i, p in enumerate(pos):
            if p == 1 and not in_pos:
                ...
                in_pos = False
                trade_ret = np.prod(1 + sign * r[entry_i:i]) - 1
                rows.append((ticker, direction, dates[entry_i], dates[i - 1], i - entry_i, trade_ret))
        if in_pos:  ## still holding at the end of the window
            trade_ret = np.prod(1 + sign * r[entry_i:]) - 1
            rows.append((ticker, direction, dates[entry_i], dates[-1], len(pos) - entry_i, trade_ret))
    return pd.DataFrame(rows, columns=['ticker', 'side', 'entry', 'exit', 'days', 'trade_ret'])
...
def plot_backtest_performance(port_ret, etf_name=None, trades=None, sharpe=None,
                              maxdd=None, endpnl=None, save_path=None, show=False,
                              title="Stat-arb backtest performance"):
    '''
    Reusable 2x2 performance dashboard for a finished backtest. Works with the
    `port_ret` DataFrame produced by bt.run()/port_performance (needs columns
    'cum_pnl', 'max_dd', 'long_pnl', 'short_pnl'); pass a trade blotter to add the
    per-trade return histogram.

    :param port_ret: (pd.DataFrame) performance frame from port_performance.
    :param etf_name: (str) optional ETF column in port_ret to overlay as buy&hold.
    :param trades: (pd.DataFrame) optional blotter with a 'trade_ret' column.
    :param sharpe/maxdd/endpnl: (float) optional headline metrics to annotate.
    :param save_path: (str) if given, save the figure (PNG) here.
    :param show: (bool) if True, display the figure interactively (plt.show()).
    :return: (matplotlib.figure.Figure) the figure, in case the caller wants it.
    '''
    fig, axs = plt.subplots(2, 2, figsize=(13, 8))
    axs = axs.flatten()

    port_ret['cum_pnl'].plot(ax=axs[0], label='strategy')
    if etf_name is not None and etf_name in port_ret.columns:
        (port_ret[etf_name] + 1).cumprod().plot(ax=axs[0], label=f'{etf_name} buy&hold')
    axs[0].axhline(1.0, lw=0.6, ls='--', color='k')
    axs[0].set_title('Cumulative PnL (equity curve)')
    axs[0].legend(loc='upper left')
    if sharpe is not None:
        axs[0].text(0.02, 0.04,
                    f"Sharpe {sharpe:.2f}\nMaxDD {maxdd:.1%}\nEndPnL {endpnl:.2f}",
                    transform=axs[0].transAxes, va='bottom', ha='left', fontsize=9,
                    bbox=dict(boxstyle='round', fc='white', ec='0.7', alpha=0.8))

    port_ret['max_dd'].plot(ax=axs[1], color='firebrick')
    axs[1].set_title('Rolling 6m max drawdown')

    port_ret[['long_pnl', 'short_pnl']].plot(ax=axs[2])
    axs[2].set_title('Long vs short leg PnL')

    if trades is not None and not trades.empty:
        axs[3].hist(trades['trade_ret'] * 100, bins=60, color='steelblue')
        axs[3].axvline(0, lw=0.8, ls='--', color='k')
    axs[3].set_title('Per-trade return distribution (%)')
